...
태어난 시간이 없으면 사주 엔진은 시간주를 제외한 해석 모드로 동작한다. 구현 초기에는 `시간 미상` 값을 별도로 모델링하고, 기존 `datetime` 강제 입력은 확장한다.
...
두 사람 메뉴에서는 두 사람의 개인정보를 먼저 입력받아 사주 DB 조회를 시작하고, 이후 한 사람씩 순차 캡처한다. 첫 번째 사람이 촬영되면 3초 뒤 두 번째 사람 촬영을 시작한다.
...
- 두 사람의 개인정보 입력 후 사주 DB 조회를 먼저 시작한다.
...
- 개인 리포트: 개인정보 입력 후 사주 DB 조회와 단일 얼굴 캡처를 병렬 실행한다.
- 두 사람 궁합: 두 사람 개인정보 입력 후 사주 DB 조회를 시작하고, 첫 번째 사람 촬영 후 3초 뒤 두 번째 사람을 순차 촬영한다.
- LLM: 관상 분석 LLM과 최종 리포트 LLM 설정을 분리한다.
- DB: SQLite 사주 캐시와 SQLite 얼굴 추천 DB를 사용한다.
...
flowchart TD
    ...
    PairInput --> PairSaju[두 사람 사주 DB 조회]
