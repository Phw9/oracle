...
RUN_ORACLE_FACE_RULEBASE_COMPARE_LLM="${RUN_ORACLE_FACE_RULEBASE_COMPARE_LLM:-0}"
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --face-rulebase)
        ...
        RUN_ORACLE_FACE_RULEBASE_COMPARE_LLM=1
        shift 1
        ;;
      --face-llm)
        RUN_ORACLE_FACE_ANALYSIS_MODE=1
        RUN_ORACLE_FACE_RULEBASE_COMPARE_LLM=0
        shift 1
        ;;
      --debug)
...
apply_run_config() {
  ...
  export ORACLE_FACE_RULEBASE_COMPARE_LLM="$RUN_ORACLE_FACE_RULEBASE_COMPARE_LLM"
